...
Color w_buff[2000][2000];
Color b_buff[2000][2000];
...
public:
    ...
    pair<int, int> getXYForTexture(Point p, int isW) {
        ...
        // double rel_x = ((int)(p.x - ref_point.x)) % (int)(widthOfEachCell);
        // double rel_y = ((int)(p.y - ref_point.y)) % (int)(widthOfEachCell);
        // double width = w_width;
        // double height = w_height;
        // if (!isW) {
        //     width = b_width;
        //     height = b_height;
        // }
        // int cell_x = (p.x - ref_point.x) / widthOfEachCell;
        // int cell_y = (p.y - ref_point.y) / widthOfEachCell;

        // int image_x = (rel_x / widthOfEachCell) * width;
        // int image_y = (rel_y / widthOfEachCell) * height;
        // if (image_x < 0 or image_y < 0) return make_pair(-1, -1);
        // if (image_x >= width or image_y >= height) return make_pair(-1, -1);
...
public:
    ...
    CheckerBoard(int _widthOfEachCell) {
        ...
        ref_point = Point(-5000, -5000, 0); // bottom left corner of the checkerboard
    }
